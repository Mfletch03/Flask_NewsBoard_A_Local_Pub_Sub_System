...
class FlaskAppTests(unittest.TestCase):
    def setUp(self):
        ...
        # Reset global state
        admins.clear()
        admins.update({"admin": "password"})
        subscribers.clear()
        subscribers.update({"test": "123"})
    ...
    def test_valid_subscriber_login(self):
        response = self.app.post('/', data={'username': 'test', 'password': '123'}, follow_redirects=True)
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Welcome, test!', response.data)

    def test_invalid_login(self):
        response = self.app.post('/', data={'username': 'wrong', 'password': 'nope'}, follow_redirects=True)
        self.assertEqual(response.status_code, 400)
        self.assertIn(b'Invalid username or password', response.data)

    def test_wrong_password(self):
        response = self.app.post('/', data={'username': 'test', 'password': 'wrong'}, follow_redirects=True)
        self.assertEqual(response.status_code, 400)
        self.assertIn(b'Incorrect password', response.data)

    # ---------- SIGNUP TESTS ----------
    def test_signup_new_user(self):
        response = self.app.post('/sign_up', data={'username': 'newuser', 'password': 'pass'}, follow_redirects=True)
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Welcome, newuser!', response.data)
        self.assertIn('newuser', subscribers)

    def test_signup_missing_fields(self):
        response = self.app.post('/sign_up', data={'username': '', 'password': ''}, follow_redirects=True)
        self.assertEqual(response.status_code, 400)
        self.assertIn(b'Please enter both username and password', response.data)

    def test_signup_existing_user(self):
        response = self.app.post('/sign_up', data={'username': 'test', 'password': 'another'}, follow_redirects=True)
        self.assertEqual(response.status_code, 400)
        self.assertIn(b'Username already exists', response.data)

    # ---------- ADMIN TESTS ----------
    def test_admin_login(self):
        response = self.app.post('/', data={'username': 'admin', 'password': 'password'}, follow_redirects=True)
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Welcome Admin admin!', response.data)

    def test_admin_post_message(self):
        response = self.app.post('/admin/admin/post_message', data={'message': 'Hello Users!'}, follow_redirects=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_change_password(self):
        ...
        self.assertEqual(subscribers['test'], 'newpass')
